export async function get_map_data(db_pool)
{
    try {
        ...
        const result = await db_query(query, db_pool);
        ...
        return JSON.parse(formatted_result);
    }
    catch {
...
export async function get_chart_data(range, db_pool)
{
    try {
        let query;
        query = "select DATE(date_time) AS time, COUNT(id) as total_connections FROM connections GROUP BY DATE(connections.date_time) ORDER BY time DESC LIMIT " + range + ";";
        const result = await db_query(query, db_pool);
        let formatted_result = '[';
