...
app.get('/get-chart-data', cors(corsOptions), async function (req, res) {
    ...
    try {
        ...
        for (let i = result.length - 1; i > 0 ; i--) {
            let date_split = result[i]['time'];
            let date_split_array = date_split.split(" ");
            let date = date_split_array[0] + " " + date_split_array[1] + " " + date_split_array[2];
            formatted_result += '{"time": "' + date + '", "data": ' + result[i]['total_connections'] + '},';
        }
        let date_split = result[0]['time'];
        let date_split_array = date_split.split(" ");
        let date = date_split_array[0] + " " + date_split_array[1] + " " + date_split_array[2];
